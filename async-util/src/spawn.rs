...
impl<T> Future for TokioJoinHandle<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        Pin::new(&mut self.0)
            .poll(cx)
            .map(|r| r.unwrap_or_else(|e: tokio::task::JoinError| resume_unwind(e.into_panic())))
    }
}
...
impl Spawn for tokio::runtime::Handle {
    ...
    fn spawn_with_handle<F: 'static + Send + Future>(&self, fut: F) -> TokioJoinHandle<F::Output>
    where
        F::Output: 'static + Send,
    {
    ...
    fn spawn<F: 'static + Send + Future<Output = ()>>(&self, fut: F) { tokio::spawn(fut); }
}
