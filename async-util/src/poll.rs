...
impl WakeGuard {
    pub fn new() -> Self {
        WakeGuard(Arc::new(WakeGuardInner {
            awake: AtomicUsize::new(1),
            waker: AtomicWaker::new(),
        }))
    }
    fn guarded<'a, 'b, 'c, E, F: for<'d> FnOnce(&'d mut Context<'d>) -> PollResult<(), E>>(
        &'a self,
        cx: &'b mut Context<'c>,
        fun: F,
    ) -> PollResult<(), E> {
...
impl<T: Hash + Eq> WakeGuardSet<T> {
    pub fn new() -> Self {
        WakeGuardSet {
            inner: Arc::new(Mutex::new(WakeGuardSetInner {
                awake: Vec::new(),
                waker: None,
            })),
        }
    ...
    pub fn push(&mut self, value: T) {
        self.inner
            ...
            .unwrap()
            .awake
            .push(Arc::new(WakeGuardSetItem {
                inner: Arc::downgrade(&self.inner),
                awake: AtomicBool::new(true),
                value,
            }));
    }
    pub fn guarded<E, F: for<'a, 'b> Fn(&'a mut Context<'b>, &T) -> PollResult<(), E>>(
        &mut self,
        cx: &mut Context,
        fun: F,
    ) -> PollResult<(), E> {
...
pub fn poll_next<S: Stream + Unpin>(cx: &mut Context, str: &mut Option<S>) -> PollResult<S::Item> {
    if let Some(str2) = str {
        match Pin::new(str2).poll_next(cx) {
            ...
            Poll::Ready(Some(next)) => Yield(next),
        }
    } else {
        Noop
    }
