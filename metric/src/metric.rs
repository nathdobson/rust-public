...
impl<K: Keys, V: Values + Clone> Metric<K, V> {
    ...
    pub fn local(&self) -> LocalMetric<K, V> {
        ...
        lock.entries.push(MetricEntry { cell: cell.clone(), cache: HashMap::new() });
        LocalMetric { stats: self.stats.clone(), cell }
    }
    pub fn get(&self, result: &mut HashMap<K, V::Set>) {
        ...
        for entry in this.entries.iter_mut() {
            ...
            for (keys, value) in entry.cache.iter() {
                self.stats.add_set(&mut this.total.entry(keys.clone()).or_default(), value);
            }
        }
...
impl<K: Keys, V: Values + Clone> LocalMetric<K, V> {
    pub fn add(&self, key: K, p: V::Point) {
        self.stats.add_point(&mut *self.cell.lock().entry(key).or_default(), &p);
    }
}

pub trait AsAny { fn as_any(&self) -> &dyn Any; }

impl<T: Sized + 'static> AsAny for T { fn as_any(&self) -> &dyn Any { self } }
