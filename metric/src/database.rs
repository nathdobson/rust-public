...
impl From<i64> for SnapshotValue { fn from(x: i64) -> Self { SnapshotValue::Integer(x) } }

impl From<u64> for SnapshotValue { fn from(x: u64) -> Self { SnapshotValue::Integer(x as i64) } }

impl From<i32> for SnapshotValue { fn from(x: i32) -> Self { SnapshotValue::Integer(x as i64) } }

impl From<u32> for SnapshotValue { fn from(x: u32) -> Self { SnapshotValue::Integer(x as i64) } }
...
impl FromIterator<SnapshotRow> for Snapshot {
    fn from_iter<T: IntoIterator<Item=SnapshotRow>>(iter: T) -> Self {
        Snapshot { rows: iter.into_iter().collect() }
    }
}
