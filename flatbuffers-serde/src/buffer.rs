use flatbuffers::{FlatBufferBuilder, WIPOffset, InvalidFlatbuffer, Follow, Verifiable, root, root_unchecked, Verifier, VerifierOptions, ForwardsUOffset};
...
impl<T> FlatBuffer<T> {
    ...
    pub fn from_vec_slice(data: VecSlice) -> Result<Self, InvalidFlatbuffer> where T: Verifiable {
        let opts = VerifierOptions::default();
        let mut v = Verifier::new(&opts, data.as_ref());
        <ForwardsUOffset<T>>::run_verifier(&mut v, 0)?;
        Ok(FlatBuffer { data, phantom: PhantomData })
    }
    pub fn into_inner(self) -> VecSlice {
        self.data
    }
    pub fn as_slice(&self) -> &[u8] {
        self.data.as_ref()
    }
    pub fn root<'a>(&'a self) -> T::Inner where T: Follow<'a> {
        unsafe { root_unchecked::<T>(self.data.as_ref()) }
    }
    pub fn root_covariant<'a>(&'a self) -> <T::Super<'a> as Follow<'a>>::Inner where T: Covariant, T::Super<'a>: Follow<'a> {
        unsafe { root_unchecked::<T::Super<'a>>(self.data.as_ref()) }
    }
}

pub unsafe trait Covariant {
    type Super<'b> where Self: 'b;
}

impl<T> Debug for FlatBuffer<T> where
    T: Covariant,
