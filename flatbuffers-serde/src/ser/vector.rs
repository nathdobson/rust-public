...
use crate::flat_util::{FlatUnit, Flat128};
use crate::ser::error;
...
impl<'a, 'b> VectorBuilder<'a, 'b> {
    pub fn new(serializer: Serializer<'a, 'b>) -> Self {
        ...
        VectorBuilder { serializer, element_start: len }
    }
    pub fn reborrow<'c>(&'c mut self) -> Serializer<'c, 'b> {
        self.serializer.reborrow()
    }
    pub fn push(&mut self, value: OneValue) {
    ...
    fn end_vector(self) -> Result<Value> {
        ...
        if let Some(head) = &head {
            match head {
                ...
                OneValue::Fixed0 => { self.serializer.fbb.start_vector::<FlatUnit>(len); }
                ...
                OneValue::Fixed128(_) => { self.serializer.fbb.start_vector::<Flat128>(len); }
            }
        } else {
        ...
        for element in iter.rev() {
            match element {
                ...
                OneValue::Fixed0 => { self.serializer.fbb.push(FlatUnit); }
                ...
                OneValue::Fixed64(x) => { self.serializer.fbb.push(x); }
                OneValue::Fixed128(x) => { self.serializer.fbb.push(Flat128(x)); }
            }
        }
        let vector = if let Some(head) = &head {
            match head {
                ...
                OneValue::Fixed0 => self.serializer.fbb.end_vector::<FlatUnit>(len).as_union_value(),
                ...
                OneValue::Fixed128(_) => self.serializer.fbb.end_vector::<Flat128>(len).as_union_value(),
            }
        } else {
            self.serializer.fbb.end_vector::<Flat128>(len).as_union_value()
        };
