...
use crate::input::{Key, KeyEvent, Mouse};
...
impl Field {
    pub fn new(tree: Tree, content: String) -> DivRc<Self> {
        DivRc::new_cyclic(tree, |this| Field {
            content,
            enabled: true,
            cursor: 0,
        })
...
impl DivImpl for Field {
    fn layout_impl(self: &mut Div<Self>, constraint: &Constraint) -> Layout {
        ...
        Layout {
            size: (max.0, 1),
            line_settings: iter::once((0, LineSetting::Normal)).collect(),
        }
    }
    fn self_handle(self: &mut Div<Self>, event: &InputEvent) -> bool {
        match event {
            ...
            InputEvent::KeyEvent(KeyEvent { modifier, key }) => match *key {
