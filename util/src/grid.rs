...
impl<T> Grid<T> {
    ...
    pub fn bounds(&self) -> Rect { Rect::from_ranges(0..self.size.0, 0..self.size.1) }
    pub fn as_ref(&self) -> GridSlice<T> { GridSlice(self.as_raw(), PhantomData) }
    pub fn as_mut(&mut self) -> GridSliceMut<T> { GridSliceMut(self.as_raw(), PhantomData) }
    ...
    //        }
    //    }
    pub fn get(&self, p: (isize, isize)) -> Option<&T> { self.vec.get(self.index_of(p)?) }
    pub fn get_mut(&mut self, p: (isize, isize)) -> Option<&mut T> {
        let index = self.index_of(p)?;
        self.vec.get_mut(index)
    }
    pub fn size(&self) -> (isize, isize) { self.size }
    ...
    pub fn points(&self) -> impl Iterator<Item = (isize, isize)> { self.bounds().points_by_row() }
...
pub trait GridSliceIndex: Sized {
    ...
    fn size(&self) -> (isize, isize) { self.as_raw().bounds.size() }
    fn cell(self) -> Self::Target;
}

pub fn rows<T: GridSliceIndex>(x: T) -> Rows<T> { Rows(Some(x)) }

pub fn cols<T: GridSliceIndex>(x: T) -> Cols<T> { Cols(Some(x)) }
...
impl<'a, T> GridSliceIndex for GridSlice<'a, T> {
    ...
    fn cell(self) -> Self::Target { unsafe { &*self.0.get_raw((0, 0)).unwrap().as_ptr() } }
}
...
impl<'a, T> GridSliceIndex for GridSliceMut<'a, T> {
    ...
    fn cell(self) -> Self::Target { unsafe { &mut *self.0.get_raw((0, 0)).unwrap().as_ptr() } }
}
...
impl<'a, T> GridSliceMut<'a, T> {
    pub fn as_mut<'b>(&'b mut self) -> GridSliceMut<'b, T> { GridSliceMut(self.0, PhantomData) }
    pub fn into_ref(self) -> GridSlice<'a, T> { GridSlice(self.0, PhantomData) }
    pub fn as_ref<'b>(&'b self) -> GridSlice<'b, T> { GridSlice(self.0, PhantomData) }
...
impl<S: GridSliceIndex> Iterator for Rows<S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut results = [
            (Rect::from_ranges(0..w, 0..1), None),
            (Rect::from_ranges(0..w, 1..h), None),
        ];
...
impl<S: GridSliceIndex> Iterator for Cols<S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut results = [
            (Rect::from_ranges(0..1, 0..h), None),
            (Rect::from_ranges(1..w, 0..h), None),
        ];
        inner.split(&mut results);
        self.0 = Some(results[1].1.take().unwrap());
        results[0].1.take()
...
impl<'a, T> IndexMut<(isize, isize)> for GridSliceMut<'a, T> {
    fn index_mut(&mut self, index: (isize, isize)) -> &mut Self::Output {
        unsafe { &mut *self.0.get_raw(index).unwrap().as_ptr() }
    }
}
...
fn test_grid_rows() {
    ...
    println!(
        "{:?}",
        rows(foo.as_ref()).map(|x| x.0.bounds).collect::<Vec<_>>()
    );
}
...
fn test_grid_cells_by_row() {
    ...
    assert_eq!(
        (0..6).map(Box::new).collect::<Vec<_>>(),
        cells_by_row(foo.as_ref()).cloned().collect::<Vec<_>>()
    );
    assert_eq!(
        (0..6).map(Box::new).collect::<Vec<_>>(),
        cells_by_row(foo.as_mut())
            .map(|x| x.clone())
            .collect::<Vec<_>>()
    );
    ...
    assert_eq!(
        Vec::<&Box<_>>::new(),
        cells_by_row(foo.as_ref()).collect::<Vec<_>>()
    );
    assert_eq!(
        Vec::<&Box<_>>::new(),
        cells_by_row(foo.as_mut()).collect::<Vec<_>>()
    );
}
