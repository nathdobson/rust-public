...
impl<K: Ord, V> RangeMap<K, V> {
    ...
    pub fn erase<'a, 'b, Q: 'b, R: RangeBounds<&'b Q> + Clone>(&'a mut self, range: R)
    where
        K: Clone + Borrow<Q>,
        Q: Ord,
    {
        loop {
            let remove: K = if let Some((r, _)) = self.range(range.clone()).next() {
                r.start.clone()
            } else {
                break;
            };
    ...
    pub fn erase_and_insert(&mut self, range: Range<K>, value: V)
    where
        K: Clone,
    {
    ...
    pub fn values(&self) -> impl Iterator<Item = &V> { self.inner.values().map(|v| &v.value) }
    pub fn range<'a, 'b: 'a, Q, R>(
        &'a self,
        range: R,
...
fn test_range_map() {
    ...
    }
    {
        let mut map = map.clone();
        map.erase(..&75);
        assert_eq!(map.iter().collect::<Vec<_>>(), vec![(&90u8..&100, &"e")]);
    }
    {
        let mut map = map.clone();
        map.erase(&35u8..);
        assert_eq!(map.iter().collect::<Vec<_>>(), vec![(&10u8..&20, &"a")]);
    }
    {
        let mut map = map.clone();
        map.erase(..);
