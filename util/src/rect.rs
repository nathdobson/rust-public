...
impl Rect {
    ...
    pub fn translate(&self, point: (isize, isize)) -> Option<(isize, isize)> {
        let (x, y) = (self.xs.0.checked_add(point.0).unwrap(), self.ys.0.checked_add(point.1).unwrap());
    ...
    pub fn sub_rectangle(&self, other: &Rect) -> Rect {
        Rect::from_ranges(self.xs().sub_range(&other.xs()),
                          self.ys().sub_range(&other.ys()))
    }
    pub fn sub_rectangle_truncated(&self, other: &Rect) -> Rect {
        Rect::from_ranges(self.xs().sub_range_truncated(&other.xs()),
