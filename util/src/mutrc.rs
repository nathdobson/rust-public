...
impl<T: ?Sized> MutRc<T> {
    pub fn new_cyclic(f: impl FnOnce(&MutWeak<T>) -> T) -> MutRc<T>
    where
        T: Sized,
    {
        MutRc(Arc::new_cyclic(|w| {
    ...
    pub fn read(&self) -> ReadGuard<T> { self.0.borrow() }
    pub fn write(&mut self) -> WriteGuard<T> { self.0.borrow_mut() }
    pub fn borrow_mut(&self) -> WriteGuard<T> { self.0.borrow_mut() }
    pub fn downgrade(&self) -> MutWeak<T> { MutWeak(Arc::downgrade(&self.0)) }
    pub fn as_ptr(&self) -> *const u8 { unsafe { mem::transmute_copy::<Self, *const u8>(self) } }
    pub fn as_inner_ptr(&self) -> *mut T { self.0.as_ptr() }
    pub fn into_raw(this: Self) -> *const AtomicRefCell<T> { Arc::into_raw(this.0) }
    pub unsafe fn from_raw(raw: *const AtomicRefCell<T>) -> Self { MutRc(Arc::from_raw(raw)) }
}
...
impl<T: ?Sized> MutWeak<T> {
    ...
    pub fn into_raw(self) -> *const AtomicRefCell<T> { self.0.into_raw() }
    pub unsafe fn from_raw(raw: *const AtomicRefCell<T>) -> Self { MutWeak(Weak::from_raw(raw)) }
}
...
impl<T: ?Sized> Clone for MutWeak<T> {
    fn clone(&self) -> Self { MutWeak(self.0.clone()) }
}
...
impl<T, U> CoerceUnsized<MutRc<U>> for MutRc<T>
where
    T: ?Sized + Unsize<U>,
    U: ?Sized,
{
...
impl<T, U> CoerceUnsized<MutWeak<U>> for MutWeak<T>
where
    T: ?Sized + Unsize<U>,
    U: ?Sized,
{
