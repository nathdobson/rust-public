...
use ustr::ustr;
use crate::variants::{VariantSet, VariantCtx, Note};
...
fn test_block() {
    let code = "let x = 3.0;\nlet y = (x * 4.2);\nreturn y;\n";
    assert_eq!(format!("{:?}", BlockParser::new().parse(code).unwrap()), code.to_string());
}
...
fn run_script(script: &str) -> HashSet<(Value, Vec<Note>)> {
    ...
    type_ctx.check(&block).unwrap_or_else(|e| panic!("Type error: {}", e));
...
fn bad(v: f64, units: impl Into<Factors>, error: impl IntoIterator<Item=&'static str>) -> (Value, Vec<Note>) {
    (Value::with_unit(v, units.into()), error.into_iter().map(|x| Note::new(ustr(x))).collect())
}
...
fn test_mul() {
    ...
    assert_eq!(run_script(script), expected([
        good(1.0, "miles"),
        bad(3600.0, "miles", ["Skipped unit conversion from `seconds mph' to `miles'."])
    ]));
}
