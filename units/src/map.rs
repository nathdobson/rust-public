...
impl<K: Ord, V> VecMap<K, V> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&K, &V)> { self.into_iter() }
    ...
    pub fn retain(&mut self, f: impl Fn(&K, &V) -> bool) { self.0.retain(|(k, v)| f(k, v)); }
}
...
impl<'a, K: Ord + 'a, V: 'a> OccupiedEntry<'a, K, V> {
    pub fn get_mut(self) -> &'a mut V { &mut self.map.0[self.index].1 }
}
