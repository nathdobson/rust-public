...
impl From<Ustr> for Factors { fn from(x: Ustr) -> Self { Factors(iter::once((x, 1)).collect()) } }

impl From<&'static str> for Factors { fn from(x: &'static str) -> Self { Self::from(ustr(x)) } }

impl<const N: usize> From<[(&'static str, i32); N]> for Factors {
    fn from(x: [(&'static str, i32); N]) -> Self {
        Factors(<[(&'static str, i32); N]>::into_iter(x).map(|(k, v)| (ustr(k), v)).collect())
    }
}
