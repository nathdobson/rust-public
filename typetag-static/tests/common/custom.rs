...
use typetag_static::{AnyDeserializer, AnySerde, AnySerializer, BoxAnySerde};
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { unimplemented!() }
}
...
impl AnySerializer for Custom {
    fn serialize_dyn_impl(self, value: &dyn AnySerde) -> Result<Self::Ok, Self::Error> {
        assert_eq!(
            value.downcast_ref::<AnyString>().unwrap(),
