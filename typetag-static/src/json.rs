...
pub trait AnyJson: 'static + Send + Sync {
    ...
    fn serialize_json<'a, 'b>(&self, serializer: &'a mut JsonSerializer<'b>, value: &dyn TraitAnySerde) -> Result<(), serde_json::Error>;
    fn deserialize_json<'a, 'de>(&self, deserializer: &'a mut JsonDeserializer<'de>) -> Result<BoxAnySerde, serde_json::Error>;
}

impl<T: Serialize + for<'de> Deserialize<'de> + 'static + HasTypeTag + TraitAnySerde> AnyJson for AnySingleton<T> {
